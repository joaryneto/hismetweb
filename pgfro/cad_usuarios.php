...
if(@$inputb['ap'] == "1")
{
	...
	else
	{
	   ...
	   if($sucesso)
	   {
		   ...
		   $RES = mysqli_query($db,"SELECT * FROM usuarios where sistema='".$_SESSION['sistema']."' and login='".$inputb['login']."'");
...
if(isset($codigo))
{
	$sucesso = mysqli_query($db,"SELECT * FROM usuarios where sistema='".$_SESSION['sistema']."' and codigo='".$codigo."'");
